...
import {
    fastReplaceAddress,
    oldFastReplaceAddress,
    putWxSchoolModule,
    putWxSchoolSystem
} from "../../../../network/http";
import {FastReplaceUrlData, OldFastReplaceUrlData, OldFastReplaceUrParams} from "@/type/schoolSystem/SchoolSystemState";
...
interface FastReplaceLayerState {
    data: OldFastReplaceUrlData | object
}
...
class FastReplaceUrlLayer extends BaseComponent<FastReplaceLayerProps, FastReplaceLayerState> {
    ...
    async handChangeSystem() {
        let { lists, beforeReplaceUrl, afterReplaceUrl } = this.state.data as OldFastReplaceUrlData
        let replaceArr = lists.map(item=>{

        })
        try {
            // await oldFastReplaceAddress({
            //     url,wsUrl
            // })
